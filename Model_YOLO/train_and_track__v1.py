...
def train_yolo(epochs=EPOCHS):
    ...
    # Plot metrics 
def plot_metrics():
    results_path = Path("runs/detect/custom/results.csv")
    if not results_path.exists():
        print("❌ No results.csv found. Did training finish?")
        return
    ...
    # Try plotting using first column as epoch index
    if "epoch" not in df.columns:
        df["epoch"] = range(len(df))  # Add artificial epoch index
